...
def check_args(args):
    ...
    if not arg.number_threads.isdigit():
        ...
        for thread_no in range(len(workdirList)):
            ...
            if detector_no<number_detectors:
                qualified_new_plan_filename=os.path.join(
                    workdirList[thread_no],
                    new_plan_filename.replace('.fits','_batch%s.fits')) 
                # Update NSEED, MSEED, set NUM_DET=1                  
                simultation_plan_table=update_sim_plan_table(detector_no)
...
def create_config(args, workdir, batch):
    ...
    qualified_config_filename = os.path.join(workdir, new_config_filename)
...
def create_isf(args,
               config_filename,workdir,batch,logdir):
    ...
    qualified_isf_filename = os.path.join(workdir, new_isf_filename)

    # Set up the args we'll be replacing or setting

    args_to_set = {}
    args_to_set["workdir"] = workdir
    args_to_set["logdir"] = logdir
...
def create_thread_dir_struct(args,number_threads):
    ...
    DirStuct = namedtuple("Directories","workdir logdir")
...
def run_pipeline_from_args(args):
    ...
    nThreads= max(1,min(int(args.nThreads),multiprocessing.cpu_count()-1))
    # Set up workdirs
    ...
    for batch in batches:
        ...
        for threadNo in range(batch.nThreads):
            workdir = workdirList[threadNo].workdir
            # logdir?
            ...
            # @TODO: Do we want serverurl?
            prodThreads.append(multiprocessing.Process(target=execute_pipeline,
                args=(arg.pipeline,isf=qualified_isf_filename,
