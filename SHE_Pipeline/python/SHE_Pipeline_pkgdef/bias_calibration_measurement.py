""" @file bias_calibration_measurement.py

    Created 28 Jun 2018

    Pipeline script for the shear bias measurement calibration pipeline.
"""
...
from SHE_Pipeline_pkgdef.package_definition import (she_prepare_configs, she_simulate_images, she_estimate_shear,
                                                    she_measure_statistics, she_measure_bias, she_measure_bias_calibration_residuals,
...
@pipeline(outputs=('shear_bias_residuals_measurements',))
def shear_bias_residual_measurement(simulation_plan,
                           config_template,
                           bfd_training_data,
                           ksb_training_data,
                           lensmc_training_data,
    ...
    shear_bias_residuals_measurements = she_measure_bias_calibration_residuals(shear_bias_statistics=shear_bias_statistics,
                                               pipeline_config=pipeline_config)

    return shear_bias_residuals_measurements
    

if __name__ == '__main__':
    from euclidwf.framework.graph_builder import build_graph
    from euclidwf.utilities import visualizer
    pydron_graph = build_graph(shear_bias_residual_measurement)
