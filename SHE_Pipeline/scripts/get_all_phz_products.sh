...
CMD='python '$HOME'/bin/dataProductRetrieval_SC8.py --username '`cat $HOME/.username.txt`' --password '`cat $HOME/.password.txt`' --project TEST --data_product DpdPhzPfOutputCatalog --query "'$QUERY'"'
echo "Command: $CMD"
eval $CMD
