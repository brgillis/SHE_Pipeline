...
#SBATCH -e bias_calibration_parallel.err            # STDERR
#SBATCH -t 0-8:0:0
...
E-Run SHE_Pipeline 0.8.16 SHE_Pipeline_RunBiasCalibrationParallel --isf $ISF --isf_args config_template $CFG_TEMPLATE_HEAD$TEMPLATE_PREFIX$TAG$TEMPLATE_POSTFIX mdb $SCRIPTDIR/EUC_MDB_MISSIONCONFIGURATION-SC456_2019-03-28T1224.00Z_01.xml --workdir $WORKDIR --config_args  SHE_CTE_EstimateShear_methods "KSB REGAUSS LensMC MomentsML" SHE_CTE_MeasureBias_archive_dir $ARCHIVE_DIR/$TAG/sens_$I SHE_CTE_MeasureStatistics_archive_dir $ARCHIVE_DIR/$TAG/sens_$I --plan_args MSEED_MIN $SEED_MIN MSEED_MAX $SEED_MAX NSEED_MIN $SEED_MIN NSEED_MAX $SEED_MAX NUM_GALAXIES $NUM_GALAXIES_PER_SEED --cluster --number_threads $NUM_THREADS
